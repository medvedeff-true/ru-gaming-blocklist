...
# IPv6 regex intentionally broad; final validation is done by ipaddress.
...
def strip_comments(line: str) -> str:
    # Не режем URL по // в http:// и https://.
    line = re.sub(r"(?<!:)//.*$", "", line)
    line = line.split("#", 1)[0]
    return line.strip()
...
def clean_possible_domain(raw: str) -> str | None:
    ...
    token = token.removeprefix("*.")
    token = token.removeprefix("address=/")
    token = token.strip("/^")
    ...
    if "@" in token:
        # e-mail адреса нам не нужны.
        return None

    if token.startswith("www.") and len(token.split(".")) > 2:
        # www оставляем как валидный поддомен. Ничего не меняем.
        pass
    ...
    tld = labels[-1]
    if len(tld) < 2 or not re.fullmatch(r"[a-z]{2,63}", tld):
        return None

    # Отсекаем частые слова из markdown/текста, которые случайно похожи на домен.
...
def extract_domains(text: str) -> set[str]:
    ...
    # dnsmasq формат: address=/domain.tld/ip
...
def slugify_game_name(name: str) -> str:
    # В вашем репозитории уже используется CamelCase/подчёркивания.
    # Для новых игр делаем безопасный человекочитаемый файл.
...
def build_alias_index(game_map: dict[str, list[str]]) -> list[tuple[str, str, str]]:
    ...
    for game, aliases in game_map.items():
        all_aliases = [game] + aliases
        for alias in all_aliases:
            norm = normalize_text_for_match(alias)
            if norm:
                index.append((game, alias, norm))
    # Длинные алиасы первыми, чтобы "league of legends" был над "lol".
...
def infer_games_from_context(
    ...
    for game, _alias, norm_alias in alias_index:
        if not norm_alias:
            continue
        # Для коротких алиасов типа lol/vrc требуем границы слова.
        if len(norm_alias) <= 3:
    ...
    if domain_game_hints:
        ...
        for game, hints in domain_game_hints.items():
            for hint in hints:
                if hint.lower() in lower_context:
                    games.add(game)
                    break
...
def sanitize_explicit_game_name(name: str) -> str | None:
    ...
    generic = {
        "hostlist", "ipset", "список доменов", "список ip", "no response", "none",
        "описание", "description", "последняя", "latest", "general", "game filter", "ipset filter",
    }
    if bad in generic:
        return None
    ...
    # Безопасный ключ для имени файла, но сохраняем узнаваемость.
    parts = re.split(r"[^A-Za-z0-9А-Яа-яЁё]+", name)
...
def infer_explicit_games_from_text(
    ...
    """Пытается достать название сервиса из шаблонных issues.

    Примеры:
    [Hostlist]:EpicGames
    Название сервиса / сайта\nEpic Games
    Service name: Fortnite
    """
    games: set[str] = set()
    sample = text[:4000]
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, sample, flags=re.IGNORECASE):
            ...
            # Обрезаем хвосты markdown/заголовков.
            raw_name = re.split(r"[#|<>{}\[\]\n\r]", raw_name, 1)[0]
...
class GitHubClient:
    ...
    def _headers(self, raw: bool = False) -> dict[str, str]:
        headers = {
            "User-Agent": "medvedeff-ru-gaming-blocklist-updater",
    ...
    def issue_comments(self, repo: str, number: int, max_comments: int) -> list[dict[str, Any]]:
        ...
        for page in range(1, pages + 1):
            data = self.get_json(
                ...
                {"per_page": min(100, max_comments), "page": page},
            )
    ...
    def repo_tree(self, repo: str, branch: str) -> list[dict[str, Any]]:
        ...
        data = self.get_json(f"/repos/{repo}/git/trees/{branch_q}", {"recursive": "1"})
...
def process_text(
    ...
    base_games = infer_explicit_games_from_text(
        source_context + "\n" + text,
        alias_index,
        domain_game_hints,
    )
    ...
    for raw_line in text.splitlines():
        ...
        games = set(base_games)
        games |= infer_games_from_context(context, alias_index, domain_game_hints)
        ...
        if kind in {"mixed", "domains"}:
            for domain in extract_domains(line):
                domain_games = set(games)
                domain_games |= infer_games_from_context(domain, alias_index, domain_game_hints)
                if trust_all and default_game:
                    domain_games.add(str(default_game))
                # Не добавляем случайные домены из issue/README, если не смогли привязать к игре.
                if not domain_games:
                    continue
        ...
        if kind in {"mixed", "ips"}:
            for ip_or_cidr in extract_ips(line):
                ...
                # IP без игрового контекста добавлять рискованно: много ложных совпадений.
                if not ip_games:
                    continue
...
def score_repository_for_discovery(repo_item: dict[str, Any]) -> int:
    text_parts = [
        repo_item.get("full_name") or "",
        repo_item.get("description") or "",
        " ".join(repo_item.get("topics") or []),
    ]
    text = normalize_text_for_match(" ".join(text_parts))
    score = 0
    strong_words = ["zapret", "hostlist", "ipset", "domainlist", "iplist", "blocklist", "gaming", "game"]
    game_words = ["roblox", "fortnite", "epic", "vrchat", "steam", "valorant", "riot", "minecraft"]
...
def raw_url_for(repo: str, branch: str, path: str) -> str:
    # raw.githubusercontent.com ожидает path с URL-экранированием по сегментам.
...
def scan_raw_files_from_repo(
    ...
    for item in tree:
        ...
        if not path:
            continue
        if should_scan_path(path, item, config):
    ...
    for item in selected:
        path = item.get("path") or ""
        url = raw_url_for(repo, branch, path)
        text = gh.get_text(url)
        if text is None:
            continue
        context = f"repo:{repo} path:{path}"
        process_text(harvested, text, context, alias_index, domain_game_hints, options)
...
def scan_issues_from_repo(
    ...
    for term in terms:
        issues = gh.search_issues(repo, term, max_pages=max_pages)
        if not issues:
            continue
        for issue in issues:
            ...
            if scan_comments and max_comments > 0:
                for comment in gh.issue_comments(repo, number, max_comments=max_comments):
                    ...
                    process_text(harvested, cbody, issue_context, alias_index, domain_game_hints, options)
                    harvested.sources_scanned += 1
...
def write_results(harvested: Harvested, config: dict[str, Any], dry_run: bool) -> None:
    ...
    domains = list(harvested.domains_global.keys())
    ips = list(harvested.ips_global.keys())
...
def main() -> int:
    ...
    parser.add_argument("--full-scan", action="store_true", help="Deeper historical scan for manual first runs")
    ...
    if args.full_scan:
        log("Full scan mode: increasing issue pages/comments limits for this run")
        config.setdefault("limits", {})["max_issue_pages_per_query"] = max(10, int(config.get("limits", {}).get("max_issue_pages_per_query", 2)))
        config.setdefault("limits", {})["max_comments_per_issue"] = max(80, int(config.get("limits", {}).get("max_comments_per_issue", 40)))
    ...
    # Сначала прямые raw-ссылки: они обычно чище и полезнее.
